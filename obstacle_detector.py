...
class ObstacleDetector:
    def __init__(self, max_distance=2000):
        self.max_distance = max_distance  # umbral de detección (mm)
    ...
    def get_obstacles(self):
        """
        Retorna una lista de obstáculos detectados en forma:
        [(X, Z), (X, Z), ...] en metros
        """
        depthFrame = self.depthQueue.get().getFrame()
        ...
        for cnt in contours:
            ...
            Z = np.min(roi_nonzero) / 1000.0  # en metros
            u = x + w // 2
            v = y + h // 2
